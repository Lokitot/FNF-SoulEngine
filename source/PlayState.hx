...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		*/
		...
		var dialogueJson:DialogueFile = null;
		var file:String = Paths.json(songName + '/dialogue'); //Checks for json/Soul Engine dialogue
		if (OpenFlAssets.exists(file)) {
			dialogueJson = DialogueBoxPsych.parseDialogue(file);
		}

		#if MODS_ALLOWED
		var file:String = Paths.modsJson(songName + '/dialogue'); //Checks for json/Soul Engine dialogue in mod folder
		...
		timeBarBG.alpha = 0;
		timeBarBG.visible = !ClientPrefs.invisibleTimeBar;
		...
		timeBar.alpha = 0;
		timeBar.visible = !ClientPrefs.invisibleTimeBar;
		add(timeBar);
		add(timeTxt);
        ...
        judgementCounter.setFormat(Paths.font("vcr.ttf"),
        20, FlxColor.WHITE, FlxTextAlign.LEFT, 
        FlxTextBorderStyle.OUTLINE, FlxColor.BLACK); 
        judgementCounter.borderSize = 2; 
        judgementCounter.borderQuality = 2; 
        judgementCounter.scrollFactor.set(); judgementCounter.cameras = [camHUD]; 
        judgementCounter.screenCenter(Y); judgementCounter.text = 
        'Sicks: ${sicks}\nGoods: ${goods}\nBads: ${bads}\nShits: 
	...
	private function popUpScore(?note:Note, ?optionalRating:Float):Void
	{
		...
		precision.y += 3;
		precision.visible = !ClientPrefs.msCounter;
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'toastie':
						if(ClientPrefs.framerate <= 60 && ClientPrefs.lowQuality && !ClientPrefs.globalAntialiasing && !ClientPrefs.maxOptimization) {
							unlock = true;
						}
					case 'debugger':
